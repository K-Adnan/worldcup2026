import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';

import '../data/world_cup_data.dart';
import '../utils/prediction_standings.dart';
import 'schedule_screen.dart';
import 'table_screen.dart';

/// Fixtures + Table like [FixturesTableScreen], with distinct purple styling for Predictor.
class PredictorFixturesTableScreen extends StatefulWidget {
  const PredictorFixturesTableScreen({
    super.key,
    required this.scheduleKey,
    required this.scheduleByDay,
    required this.teams,
    required this.onRefresh,
    this.onEditStateChanged,
    this.onPrimaryTabChanged,
  });

  final GlobalKey<ScheduleScreenState> scheduleKey;
  final List<DaySchedule> scheduleByDay;
  final List<TeamInfo> teams;
  final Future<void> Function() onRefresh;
  final VoidCallback? onEditStateChanged;
  final ValueChanged<int>? onPrimaryTabChanged;

  @override
  State<PredictorFixturesTableScreen> createState() =>
      _PredictorFixturesTableScreenState();
}

class _PredictorFixturesTableScreenState extends State<PredictorFixturesTableScreen>
    with SingleTickerProviderStateMixin {
  late TabController _tabController;

  static const Color _accentBg = Color(0xFFF3E5F5);
  static const Color _contentBg = Color(0xFFEDE7F6);

  @override
  void initState() {
    super.initState();
    _tabController = TabController(length: 2, vsync: this);
    _tabController.addListener(_notifyTab);
    WidgetsBinding.instance.addPostFrameCallback((_) {
      widget.onPrimaryTabChanged?.call(_tabController.index);
    });
  }

  void _notifyTab() {
    if (_tabController.indexIsChanging) return;
    widget.onPrimaryTabChanged?.call(_tabController.index);
  }

  @override
  void dispose() {
    _tabController.removeListener(_notifyTab);
    _tabController.dispose();
    super.dispose();
  }

  @override
  Widget build(BuildContext context) {
    final tableSchedule =
        PredictionStandings.tableSchedule(widget.scheduleByDay, widget.teams);

    return Column(
      children: [
        Container(
          padding: const EdgeInsets.symmetric(vertical: 8),
          decoration: BoxDecoration(
            color: _accentBg,
            border: Border(
              bottom: BorderSide(color: Colors.deepPurple.shade100, width: 1),
            ),
            boxShadow: [
              BoxShadow(
                color: Colors.deepPurple.withValues(alpha: 0.08),
                blurRadius: 10,
                offset: const Offset(0, 2),
              ),
            ],
          ),
          child: TabBar(
            controller: _tabController,
            labelColor: const Color(0xFF4A148C),
            unselectedLabelColor: Colors.deepPurple.shade200,
            labelStyle: GoogleFonts.poppins(
              fontWeight: FontWeight.w800,
              fontSize: 13,
              letterSpacing: 0.5,
            ),
            unselectedLabelStyle: GoogleFonts.poppins(
              fontWeight: FontWeight.w600,
              fontSize: 13,
            ),
            indicatorSize: TabBarIndicatorSize.label,
            indicator: UnderlineTabIndicator(
              borderSide: BorderSide(
                width: 4,
                color: Colors.deepPurple.shade400,
              ),
              insets: const EdgeInsets.symmetric(horizontal: 16),
            ),
            tabs: const [
              Tab(
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    Icon(Icons.event_rounded, size: 20),
                    SizedBox(width: 8),
                    Text('FIXTURES'),
                  ],
                ),
              ),
              Tab(
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    Icon(Icons.table_chart_rounded, size: 20),
                    SizedBox(width: 8),
                    Text('TABLE'),
                  ],
                ),
              ),
            ],
          ),
        ),
        Expanded(
          child: Container(
            color: _contentBg,
            child: TabBarView(
              controller: _tabController,
              children: [
                ScheduleScreen(
                  key: widget.scheduleKey,
                  scheduleByDay: widget.scheduleByDay,
                  teams: widget.teams,
                  onRefresh: widget.onRefresh,
                  onEditStateChanged: widget.onEditStateChanged,
                  predictorMode: true,
                  scaffoldBackgroundColor: _contentBg,
                ),
                TableScreen(
                  scheduleByDay: tableSchedule,
                  teams: widget.teams,
                ),
              ],
            ),
          ),
        ),
      ],
    );
  }
}
